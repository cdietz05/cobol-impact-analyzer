...
| `WV-CUTBLA-KEY` | MEDIUM | second `03` — length grows `42 -> 65` (20 for the name plus 3 for the value) |
| `WV-CUTBLA-VIEW` | MEDIUM | the `01` — length grows `50 -> 73`; **note:** overlays `01 WV-DATA PIC X(4096)` — 4096 bytes, still fits |
| `WV-DATA` | *no change* | the trace climbs `05 -> 03 -> 01`, crosses the REDEFINES, and finds `WV-DATA` already reserves 4096 bytes — far more than the grown 73. It is **named in the note above**, not reported as an edit. |
...
`PIC X(60)` here it would be reported `X(60) -> X(73)`. A wide buffer that
