...
_Generated 2026-09-25 18:48:53Z._
...
- **CS-SVC-TECH-NOTES** (line 11, HIGH): `05 CS-SVC-TECH-NOTES PIC X(60) DISPLAY` → `05 CS-SVC-TECH-NOTES PIC X(66).`
- **CS-SVC-ORDER-REC** — MEDIUM — CS-SVC-ORDER-REC: record length grows 109 -> 125 bytes: Rebuild the record layout, then recompile every program that copies this group and reload any file written with the old length.
...
- **CS-XTRACT-REC** — MEDIUM — CS-XTRACT-REC: record length grows 87 -> 98 bytes: Rebuild the record layout, then recompile every program that copies this group and reload any file written with the old length.

### `examples/css/src/csbillcyc.pco`

- **EXTRACT-RECORD** (line 12, CRITICAL): `01 EXTRACT-RECORD PIC X(80) DISPLAY` → `01 EXTRACT-RECORD PIC X(98).`
...
- **LK-DISPLAY-KEY** (line 16, CRITICAL): `01 LK-DISPLAY-KEY PIC X(40) DISPLAY` → `01 LK-DISPLAY-KEY PIC X(81).`
...
- **LK-TECH-NOTES** (line 13, CRITICAL): `01 LK-TECH-NOTES PIC X(60) DISPLAY` → `01 LK-TECH-NOTES PIC X(66).`
...
- **CSACCTUPD** — includes CSACCT.cpy, CSCUST.cpy
